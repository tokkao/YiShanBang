 ...
 * Describe:他的动态页面（全部动态）
 */
public class UserAllDynamicFragment extends LazyBaseFragments {

    @BindView(R.id.rlv_user_all_dynamic)
    RecyclerView rlvUserAllDynamic;
    @BindView(R.id.llyt_no_data)
    LinearLayout llytNoData;
    @BindView(R.id.refresh_layout)
    SmartRefreshLayout refreshLayout;
    private UserAllDynamicAdapter mUserAllDynamicAdapter;//全部动态适配器
    private int mPage = 1;
    ...
    public View bindLayout(LayoutInflater inflater) {
        mRootView = inflater.inflate(R.layout.frag_user_all_dynamic, null);
        return mRootView;
    }
    ...
    public void initView() {
        ...
        //所有动态
        mUserAllDynamicAdapter = new UserAllDynamicAdapter(mContext, isMine, mType);

        rlvUserAllDynamic.setLayoutManager(new LinearLayoutManager(mContext));
        rlvUserAllDynamic.setAdapter(mUserAllDynamicAdapter);
        ...
        mUserAllDynamicAdapter.setOnDeleteDynamicListener(new UserAllDynamicAdapter.OnDeleteDynamicListener() {
            @Override
    ...
    private void getList() {
        BaseOkHttpClient.newBuilder()
            ...
            public void onSuccess(String json, String msg) {
                ...
                mUserAllDynamicAdapter.refreshList(userDynamicBeans);
                if (mUserAllDynamicAdapter.getItemCount() > 0) {
                    llytNoData.setVisibility(View.GONE);
                    rlvUserAllDynamic.setVisibility(View.VISIBLE);
                } else {
                    llytNoData.setVisibility(View.VISIBLE);
                    rlvUserAllDynamic.setVisibility(View.GONE);
                }
