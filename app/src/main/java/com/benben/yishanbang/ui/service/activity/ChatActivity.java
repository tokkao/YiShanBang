...
//        mChatAdapter.initMediaPlayer();
    ...
    private void initEmoticonsKeyBoardBar() {
        ...
        ekBar.getBtnSend().setOnClickListener(v -> {
            scrollToBottom();
            isHaveMessageCount(false,0,null);
    ...
    //剩余消息数量
    private void isHaveMessageCount(boolean isGift, int giftPrice,Message imageMessage) {
        BaseOkHttpClient.newBuilder()
                .addParam("mobile", MyApplication.mPreferenceProvider.getIMUserName())
                .addParam("mobileTwo", mTargetId)
                .addParam("moeny", isGift ? giftPrice : 0)
                .url(NetUrlUtils.REMAIN_CHAT_TIMES)
                .post()
                .build().enqueue(mContext, new BaseCallBack<String>() {
            @Override
            public void onSuccess(String json, String msg) {
                if(StringUtils.isEmpty(json)){
                    ToastUtils.show(mContext,"您没有沟通次数，无法发送");
                    return;
                }
                //是否有剩余消息条数
    ...
    private void sendTextMessage() {
        ...
        if (mAtAll) {
            message = mConv.createSendMessageAtAllMember(content, null);
            mAtAll = false;
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.jmui_return_btn) {
            returnBtn();
        } else if (i == R.id.jmui_right_btn) {//如果是聊天室
            if (isChatRoom) {
        ...
        } else if (i == R.id.jmui_recharge) {
            startActivity(new Intent(mContext, RechargeIMActivity.class).putExtra("targetId",mTargetId));
        } else if (i == R.id.rlyt_shop) {
            getShopId();
        }
    }
    ...
    public void onEventMainThread(ImageEvent event) throws IOException {
        Intent intent;
        Log.e("onEventMainThread", "onEventMainThread: event.getFlag()=" + event.getFlag());
        ...
        switch (event.getFlag()) {
            ...
            //  ToastUtil.shortToast(mContext, "该功能正在添加中");
            //  break;
            case JGApplication.BUSINESS_CAKE://cake
                Message cake = mConv.createSendImageMessage(ImageUtils.resToFile(mContext, R.mipmap.ic_gift_cake));
                isHaveMessageCount(true, 1,cake);

                break;
            case JGApplication.BUSINESS_FLOWER://flower
                Message flower = mConv.createSendImageMessage(ImageUtils.resToFile(mContext, R.mipmap.ic_gift_flower));
                isHaveMessageCount(true, 5,flower);

                break;
            case JGApplication.BUSINESS_BEAR://bear
                Message bear = mConv.createSendImageMessage(ImageUtils.resToFile(mContext, R.mipmap.ic_gift_bear));
                isHaveMessageCount(true, 20,bear);

                break;
            case JGApplication.BUSINESS_RING://ring
                Message ring = mConv.createSendImageMessage(ImageUtils.resToFile(mContext, R.mipmap.ic_gift_ring));
                isHaveMessageCount(true, 50,ring);
                break;
            default:
                break;
        }
